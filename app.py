...
def view_dashboard():
    ...
    return render_template(
        "dashboard.html", user=session["username"], total_mahasiswa=total_mahasiswa
    )
...
def view_add_mahasiswa():
    if request.method == "POST":
        ...
        if not nama or not nim:
            ...
            return render_template(
                "mahasiswa/add.html", mahasiswa={"nama": nama, "nim": nim}
            )
        if models.add_mahasiswa_db(nama, nim):
            ...
            flash(
                f"Gagal menambahkan mahasiswa. NIM '{nim}' mungkin sudah ada.", "danger"
            )
            return render_template(
                "mahasiswa/add.html", mahasiswa={"nama": nama, "nim": nim}
            )
...
def view_update_mahasiswa(mahasiswa_id):
    ...
    if request.method == "POST":
        ...
        if not nama or not nim:
            flash("Nama dan NIM tidak boleh kosong.", "warning")
            return render_template(
                "mahasiswa/update.html",
                mahasiswa={"id": mahasiswa_id, "nama": nama, "nim": nim},
            )
        if models.update_mahasiswa_db(mahasiswa_id, nama, nim):
            ...
            flash(
                f"Gagal mengupdate mahasiswa. NIM '{nim}' mungkin sudah digunakan mahasiswa lain.",
                "danger",
            )
            return render_template(
                "mahasiswa/update.html",
...
app.add_url_rule(
    "/login", endpoint="login", view_func=view_login, methods=["GET", "POST"]
)
...
app.add_url_rule(
    "/mahasiswa/", endpoint="mahasiswa_index", view_func=view_mahasiswa_index
)
app.add_url_rule(
    "/mahasiswa/add",
    endpoint="add_mahasiswa",
    view_func=view_add_mahasiswa,
    methods=["GET", "POST"],
)
app.add_url_rule(
    "/mahasiswa/delete/<int:mahasiswa_id>",
    endpoint="delete_mahasiswa",
    view_func=view_delete_mahasiswa,
    methods=["GET", "POST"],
)
app.add_url_rule(
    "/mahasiswa/update/<int:mahasiswa_id>",
    endpoint="update_mahasiswa",
    view_func=view_update_mahasiswa,
    methods=["GET", "POST"],
)
